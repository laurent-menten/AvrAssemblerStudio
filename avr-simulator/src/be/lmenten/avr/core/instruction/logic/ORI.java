	...
	extends Instruction_Rd4K8
{
	...
	public ORI( UpperRegister rd, int K )
	{
		super( rd, K );

		int opc = InstructionSet.ORI.getOpcodeMaskValue();
		opc = InstructionSet.ORI.insertOperand( OperandType.d, opc, rd.getOperandIndex() );
		opc = InstructionSet.ORI.insertOperand( OperandType.K, opc, K );
		setOpcode( opc );
	}

	public static ORI newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.ORI.extractOperand( OperandType.d, opcode );
		int K = InstructionSet.ORI.extractOperand( OperandType.K, opcode );

		return new ORI( UpperRegister.lookup( d ), K );
	}
