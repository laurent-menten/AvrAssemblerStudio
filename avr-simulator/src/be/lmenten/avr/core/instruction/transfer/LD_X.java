...
(
	...
	statusRegister = "--------",
	syntax = "LD Rd, X",
	description = "Load indirect from data space using index X"
)
public class LD_X
	extends LD
{
	...
	public LD_X( Register rd, RegisterIndexXYZ rr )
	{
		super( rd, rr );
	}

	public static LD_X newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.LD.extractOperand( OperandType.d, opcode );
		int r = InstructionSet.LD.extractOperand( OperandType.r, opcode );

		return new LD_X( Register.lookup( d ), RegisterIndexXYZ.lookup( r ) );
	}
