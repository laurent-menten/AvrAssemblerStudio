...
(
	...
	statusRegister = "--------",
	syntax = "LD Rd, X+",
	description = "Load indirect from data space using index X post-incremented"
)
public class LD_Xp
	extends LD
{
	...
	public LD_Xp( Register rd, RegisterIndexXYZ rr )
	{
		super( rd, rr );
	}

	public static LD_Xp newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.LD.extractOperand( OperandType.d, opcode );
		int r = InstructionSet.LD.extractOperand( OperandType.r, opcode );

		return new LD_Xp( Register.lookup( d ), RegisterIndexXYZ.lookup( r ) );
	}
