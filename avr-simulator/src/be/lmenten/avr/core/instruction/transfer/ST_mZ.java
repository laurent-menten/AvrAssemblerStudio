...
(
	...
	statusRegister = "--------",
	syntax = "ST -Z, Rd",
	description = "Store indirect to data space using index Z pre-decremented"
)
public class ST_mZ
	extends ST
{
	...
	public ST_mZ( RegisterIndexXYZ rd, Register rr )
	{
		super( rd, rr );
	}

	public static ST_mZ newInstance( Integer opcode, Integer opcode2 )
	{
		int r = InstructionSet.ST.extractOperand( OperandType.r, opcode );
		int d = InstructionSet.ST.extractOperand( OperandType.d, opcode );

		return new ST_mZ( RegisterIndexXYZ.lookup( d ), Register.lookup( r ) );
	}
