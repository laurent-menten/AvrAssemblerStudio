...
(
	...
	statusRegister = "--------",
	syntax = "ST X+, Rd",
	description = "Store indirect to data space using index X post-incremented"
)
public class ST_Xp
	extends ST
{
	...
	public ST_Xp( RegisterIndexXYZ rd, Register rr )
	{
		super( rd, rr );
	}

	public static ST_Xp newInstance( Integer opcode, Integer opcode2 )
	{
		int r = InstructionSet.ST.extractOperand( OperandType.r, opcode );
		int d = InstructionSet.ST.extractOperand( OperandType.d, opcode );

		return new ST_Xp( RegisterIndexXYZ.lookup( d ), Register.lookup( r ) );
	}
