...
(
	...
	statusRegister = "--------",
	syntax = "LD Rd, -X",
	description = "Load indirect from data space using index X pre-decremented"
)
public class LD_mX
	extends LD
{
	...
	public LD_mX( Register rd, RegisterIndexXYZ rr )
	{
		super( rd, rr );
	}

	public static LD_mX newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.LD.extractOperand( OperandType.d, opcode );
		int r = InstructionSet.LD.extractOperand( OperandType.r, opcode );

		return new LD_mX( Register.lookup( d ), RegisterIndexXYZ.lookup( r ) );
	}
