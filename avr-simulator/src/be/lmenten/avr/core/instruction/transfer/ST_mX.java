...
(
	...
	statusRegister = "--------",
	syntax = "ST -X, Rd",
	description = "Store indirect to data space using index X pre-decremented"
)
public class ST_mX
	extends ST
{
	...
	public ST_mX( RegisterIndexXYZ rd, Register rr )
	{
		super( rd, rr );
	}

	public static ST_mX newInstance( Integer opcode, Integer opcode2 )
	{
		int r = InstructionSet.ST.extractOperand( OperandType.r, opcode );
		int d = InstructionSet.ST.extractOperand( OperandType.d, opcode );

		return new ST_mX( RegisterIndexXYZ.lookup( d ), Register.lookup( r ) );
	}
