...
(
	...
	statusRegister = "--------",
	syntax = "LD Rd, Y+",
	description = "Load indirect from data space using index Y post-incremented"
)
public class LD_Yp
	extends LD
{
	...
	public LD_Yp( Register rd, RegisterIndexXYZ rr )
	{
		super( rd, rr );
	}

	public static LD_Yp newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.LD.extractOperand( OperandType.d, opcode );
		int r = InstructionSet.LD.extractOperand( OperandType.r, opcode );

		return new LD_Yp( Register.lookup( d ), RegisterIndexXYZ.lookup( r ) );
	}
