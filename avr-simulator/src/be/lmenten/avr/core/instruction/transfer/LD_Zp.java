...
(
	...
	statusRegister = "--------",
	syntax = "LD Rd, Z+",
	description = "Load indirect from data space using index Z post-incremented"
)
public class LD_Zp
	extends LD
{
	...
	public LD_Zp( Register rd, RegisterIndexXYZ rr )
	{
		super( rd, rr );
	}

	public static LD_Zp newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.LD.extractOperand( OperandType.d, opcode );
		int r = InstructionSet.LD.extractOperand( OperandType.r, opcode );

		return new LD_Zp( Register.lookup( d ), RegisterIndexXYZ.lookup( r ) );
	}
