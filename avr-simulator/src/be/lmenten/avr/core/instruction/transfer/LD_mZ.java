...
(
	...
	statusRegister = "--------",
	syntax = "LD Rd, -Z",
	description = "Load indirect from data space using index Z pre-decremented"
)
public class LD_mZ
	extends LD
{
	...
	public LD_mZ( Register rd, RegisterIndexXYZ rr )
	{
		super( rd, rr );
	}

	public static LD_mZ newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.LD.extractOperand( OperandType.d, opcode );
		int r = InstructionSet.LD.extractOperand( OperandType.r, opcode );

		return new LD_mZ( Register.lookup( d ), RegisterIndexXYZ.lookup( r ) );
	}
