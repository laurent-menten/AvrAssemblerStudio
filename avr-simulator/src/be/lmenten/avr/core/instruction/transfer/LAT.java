	...
	extends Instruction_Rd5
{
	...
	public LAT( Register rd )
	{
		super( rd );

		int opc = InstructionSet.LAT.getOpcodeMaskValue();
		opc = InstructionSet.LAT.insertOperand( OperandType.d, opc, rd.getOperandIndex() );
		setOpcode( opc );
	}

	public static LAT newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.LAT.extractOperand( OperandType.d, opcode );

		return new LAT( Register.lookup( d ) );
	}
