...
(
	...
	statusRegister = "--------",
	syntax = "ST -Y, Rd",
	description = "Store indirect to data space using index Y pre-decremented"
)
public class ST_mY
	extends ST
{
	...
	public ST_mY( RegisterIndexXYZ rd, Register rr )
	{
		super( rd, rr );
	}

	public static ST_mY newInstance( Integer opcode, Integer opcode2 )
	{
		int r = InstructionSet.ST.extractOperand( OperandType.r, opcode );
		int d = InstructionSet.ST.extractOperand( OperandType.d, opcode );

		return new ST_mY( RegisterIndexXYZ.lookup( d ), Register.lookup( r ) );
	}
