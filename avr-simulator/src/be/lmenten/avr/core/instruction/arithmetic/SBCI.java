	...
	extends Instruction_Rd4K8
{
	...
	public SBCI( UpperRegister rd, int K )
	{
		super( rd, K );

		int opc = InstructionSet.SBCI.getOpcodeMaskValue();
		opc = InstructionSet.SBCI.insertOperand( OperandType.d, opc, rd.getOperandIndex() );
		opc = InstructionSet.SBCI.insertOperand( OperandType.K, opc, K );
		setOpcode( opc );
	}

	public static SBCI newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.SBCI.extractOperand( OperandType.d, opcode );
		int K = InstructionSet.SBCI.extractOperand( OperandType.K, opcode );

		return new SBCI( UpperRegister.lookup( d ), K );
	}
	...
	public void execute( Core core )
	{
		...
		Value r = rd.compute( (a,b,c) -> { return (byte)(a - b - c); }, K, sreg.c() );
