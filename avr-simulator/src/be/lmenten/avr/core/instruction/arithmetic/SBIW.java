...
(
	...
	statusRegister = "---*****",
	syntax = "SBIW Rd+1:Rd[24,26,28,30], K[0..63]",
	description = "Subrtact immediate from word"
)
public class SBIW
...
{	
	...
	public SBIW( UpperRegisterPair rd, int K )
	{
		super( rd, K );

		int opc = InstructionSet.SBIW.getOpcodeMaskValue();
		opc = InstructionSet.SBIW.insertOperand( OperandType.d, opc, rd.getOperandIndex() );
		opc = InstructionSet.SBIW.insertOperand( OperandType.K, opc, getImmediate() );
		setOpcode( opc );
	}

	public static SBIW newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.SBIW.extractOperand( OperandType.d, opcode );
		int K = InstructionSet.SBIW.extractOperand( OperandType.K, opcode );

		return new SBIW( UpperRegisterPair.lookup( d ), K );
	}
