	...
	extends Instruction_Rd4K8
{
	...
	public CPI( UpperRegister rd, int K )
	{
		super( rd, K );

		int opc = InstructionSet.CPI.getOpcodeMaskValue();
		opc = InstructionSet.CPI.insertOperand( OperandType.d, opc, rd.getOperandIndex() );
		opc = InstructionSet.CPI.insertOperand( OperandType.K, opc, K );
		setOpcode( opc );
	}

	public static CPI newInstance( Integer opcode, Integer opcode2 )
	{
		int d = InstructionSet.CPI.extractOperand( OperandType.d, opcode );
		int K = InstructionSet.CPI.extractOperand( OperandType.K, opcode );

		return new CPI( UpperRegister.lookup( d ), K );
	}
